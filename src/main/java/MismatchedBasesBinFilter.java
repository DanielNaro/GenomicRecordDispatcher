...
public class MismatchedBasesBinFilter implements Filter {
    ...
    public String getSQLFilter(Set<String> availableColumns, Set<String> availableTables) {
        if (availableTables.contains("MismatchedBasesBin")) {
            return "AUsMain.id == MismatchedBasesBin.au_id AND " + generateRangeIntersect(
                    Double.toString(min),
                    Double.toString(max),
                    "MismatchedBasesBin.MismatchedBasesBin_min",
                    "MismatchedBasesBin.MismatchedBasesBin_max"
            );
        } else {
            return "";
        }

...
public class AlignmentScoreBinRule implements BinRule<AlignmentScoreBin> {
    private final int[] binMaxValue;

    public AlignmentScoreBinRule(int[] binMaxValue) {
        if(binMaxValue == null){
        ...
        AlignmentScoreBin.setRule(this);
    }
    ...
    @Override
    public AlignmentScoreBin[] getTag(GlobalRecord globalRecord) {
        SAMRecord[] bestRecord = globalRecord.getBestRecords();

        AlignmentScoreBin[] result = new AlignmentScoreBin[bestRecord.length];
        int size = 0;
        for(int record_i=0; record_i < bestRecord.length; record_i++){
            if(bestRecord[record_i] != null){
                ...
                int bin = findBin(alignmentScore, binMaxValue);
                result[size] = new AlignmentScoreBin(bin);
                size++;
            }
        }

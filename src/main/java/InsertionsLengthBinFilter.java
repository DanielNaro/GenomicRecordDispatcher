...
public class InsertionsLengthBinFilter implements Filter {
    ...
    public String getSQLFilter(Set<String> availableColumns, Set<String> availableTables) {
        if(availableTables.contains("InsertionsLengthBin")) {
            return "AUsMain.id == InsertionsLengthBin.au_id AND " + generateRangeIntersect(
                    Double.toString(min),
                    Double.toString(max),
                    "InsertionsLengthBin.InsertionsLengthBin_min",
                    "InsertionsLengthBin.InsertionsLengthBin_max"
            );
        } else {
            return "";
        }

...
public class MismatchedBasesBinFilter implements Filter {
    private final int min;
    private final int max;
    private final boolean forAllReads;

    public MismatchedBasesBinFilter(int min, int max, boolean forAllReads) {
        this.min = min;
        this.max = max;
    ...
    public boolean matchesFilter(GlobalRecord globalRecord) {
        ...
        for(int record_i=0; record_i < bestRecord.length; record_i++){
            if(bestRecord[record_i] != null){
                int mappedBases = globalRecord.getTotalSizeMismatchOperations()[record_i];

                if(min <= mappedBases && mappedBases < max){
                    trueForOneRead = true;
                } else {
    ...
    public String getSQLFilter(Set<String> availableColumns, Set<String> availableTables) {
        if (availableTables.contains("es.upc.dmag.dispatcher.MismatchedBasesBin")) {
            return "AUsMain.id == es.upc.dmag.dispatcher.MismatchedBasesBin.au_id AND " + generateRangeIntersect(
                    Double.toString(min),
                    Double.toString(max),
                    "es.upc.dmag.dispatcher.MismatchedBasesBin.MismatchedBasesBin_min",
                    "es.upc.dmag.dispatcher.MismatchedBasesBin.MismatchedBasesBin_max"
            );
        } else {
            return "";
        }

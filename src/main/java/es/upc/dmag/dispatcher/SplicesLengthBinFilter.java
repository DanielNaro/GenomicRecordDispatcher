...
public class SplicesLengthBinFilter implements Filter {
    ...
    public String getSQLFilter(Set<String> availableColumns, Set<String> availableTables) {
        if(availableTables.contains("es.upc.dmag.dispatcher.SplicesLengthBin")) {
            return "AUsMain.id == es.upc.dmag.dispatcher.SplicesLengthBin.au_id AND " + generateRangeIntersect(
                    Double.toString(min),
                    Double.toString(max),
                    "es.upc.dmag.dispatcher.SplicesLengthBin.SplicesLengthBin_min",
                    "es.upc.dmag.dispatcher.SplicesLengthBin.SplicesLengthBin_max"
            );
        } else {
            return "";
        }

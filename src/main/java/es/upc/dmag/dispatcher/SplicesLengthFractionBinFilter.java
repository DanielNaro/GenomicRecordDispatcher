...
public class SplicesLengthFractionBinFilter implements Filter{
    ...
    public String getSQLFilter(Set<String> availableColumns, Set<String> availableTables) {
        if(availableTables.contains("es.upc.dmag.dispatcher.SplicesLengthFractionBin")) {
            return "AUsMain.id == es.upc.dmag.dispatcher.SplicesLengthFractionBin.au_id AND " + generateRangeIntersect(
                    Double.toString(min),
                    Double.toString(max),
                    "es.upc.dmag.dispatcher.SplicesLengthFractionBin.SplicesLengthFractionBin_min",
                    "es.upc.dmag.dispatcher.SplicesLengthFractionBin.SplicesLengthFractionBin_max"
            );
        } else {
            return "";
        }

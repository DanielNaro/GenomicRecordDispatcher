...
public class MaximalQualityBinFilter implements Filter {
    private final int min;
    private final int max;
    private final boolean forAllReads;

    public MaximalQualityBinFilter(int min, int max, boolean forAllReads) {
        this.min = min;
        this.max = max;
    ...
    public boolean matchesFilter(GlobalRecord globalRecord) {
        ...
        for(int record_i=0; record_i < bestRecord.length; record_i++){
            if(bestRecord[record_i] != null){
                int maximalQuality = globalRecord.getMaximalQuality();

                if(min <= maximalQuality && maximalQuality < max){
                    trueForOneRead = true;
                } else {
    ...
    public String getSQLFilter(Set<String> availableColumns, Set<String> availableTables) {
        if(availableTables.contains("es.upc.dmag.dispatcher.MaximalQualityBin")) {
            return "AUsMain.id == es.upc.dmag.dispatcher.MaximalQualityBin.au_id AND " + generateRangeIntersect(
                    Double.toString(min),
                    Double.toString(max),
                    "es.upc.dmag.dispatcher.MaximalQualityBin.MaximalQualityBin_min",
                    "es.upc.dmag.dispatcher.MaximalQualityBin.MaximalQualityBin_max"
            );
        }else{
            return "";
        }

...
public class NumInsertionsFractionBinFilter implements Filter {
    ...
    public String getSQLFilter(Set<String> availableColumns, Set<String> availableTables) {
        if(availableTables.contains("es.upc.dmag.dispatcher.NumInsertionsFractionBin")) {
            return "AUsMain.id == es.upc.dmag.dispatcher.NumInsertionsFractionBin.au_id AND " + generateRangeIntersect(
                    Double.toString(min),
                    Double.toString(max),
                    "es.upc.dmag.dispatcher.NumInsertionsFractionBin.NumInsertionsFractionBin_min",
                    "es.upc.dmag.dispatcher.NumInsertionsFractionBin.NumInsertionsFractionBin_max"
            );
        } else {
            return "";
        }

...
public class NumSplicesBinFilter implements Filter {
    ...
    public String getSQLFilter(Set<String> availableColumns, Set<String> availableTables) {
        if(availableTables.contains("es.upc.dmag.dispatcher.NumSplicesBin")) {
            return "AUsMain.id == es.upc.dmag.dispatcher.NumSplicesBin.au_id AND " + generateRangeIntersect(
                    Double.toString(min),
                    Double.toString(max),
                    "es.upc.dmag.dispatcher.NumSplicesBin.NumSplicesBin_min",
                    "es.upc.dmag.dispatcher.NumSplicesBin.NumSplicesBin_max"
            );
        } else {
            return "";
        }
    ...
    public String getExtraTable() {
        return "es.upc.dmag.dispatcher.NumSplicesBin";
    }
}

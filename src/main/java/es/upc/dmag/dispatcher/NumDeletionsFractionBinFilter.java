...
public class NumDeletionsFractionBinFilter implements Filter {
    ...
    public String getSQLFilter(Set<String> availableColumns, Set<String> availableTables) {
        if(availableTables.contains("es.upc.dmag.dispatcher.NumDeletionsFractionBin")) {
            return "AUsMain.id == es.upc.dmag.dispatcher.NumDeletionsFractionBin.au_id AND " + generateRangeIntersect(
                    Double.toString(min),
                    Double.toString(max),
                    "es.upc.dmag.dispatcher.NumDeletionsFractionBin.NumDeletionsFractionBin_min",
                    "es.upc.dmag.dispatcher.NumDeletionsFractionBin.NumDeletionsFractionBin_max"
            );
        } else {
            return "";
        }

...
public class NumInsertionsBinFilter implements Filter {
    ...
    public String getSQLFilter(Set<String> availableColumns, Set<String> availableTables) {
        if(availableTables.contains("es.upc.dmag.dispatcher.NumInsertionsBin")) {
            return "AUsMain.id == es.upc.dmag.dispatcher.NumInsertionsBin.au_id AND " + generateRangeIntersect(
                    Double.toString(min),
                    Double.toString(max),
                    "es.upc.dmag.dispatcher.NumInsertionsBin.NumInsertionsBin_min",
                    "es.upc.dmag.dispatcher.NumInsertionsBin.NumInsertionsBin_max"
            );
        } else {
            return "";
        }

...
public class SplicesLengthBinRule implements BinRule<SplicesLengthBin>{
    private final int[] binMaxValue;

    public SplicesLengthBinRule(int[] binMaxValue) {
        if(binMaxValue == null){
        ...
        SplicesLengthBin.setRule(this);
    }

    @Override
    public SplicesLengthBin[] getTag(GlobalRecord globalRecord) {
        SAMRecord[] bestRecord = globalRecord.getBestRecords();

        SplicesLengthBin[] result = new SplicesLengthBin[bestRecord.length];
        int size = 0;
        for(int record_i=0; record_i < bestRecord.length; record_i++){
            if(bestRecord[record_i] != null){
                ...
                int bin = findBin(numSplicedBases, binMaxValue);
                result[size] = new SplicesLengthBin(bin);
                size++;
            }
        }
    ...
    public String getName() {
        return "es.upc.dmag.dispatcher.SplicesLengthBin";
    }
